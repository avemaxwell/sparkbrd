function transform(url: string, width: number, quality: number): string {
  ...
  // Seed images are already downloaded from Unsplash at 1080px — no transform needed.
  // Skipping them eliminates the largest share of transformation billing.
  if (url.includes("/tacks/seeds/")) return url;
